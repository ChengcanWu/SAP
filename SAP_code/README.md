...
1. **`pq.read_table` in `main.py`** uses PyArrow’s `parquet` module. If you see `name 'pq' is not defined`, add at the top of `main.py`:  
   `import pyarrow.parquet as pq`
2. Training expects **CUDA** and enough memory; on OOM, reduce `batch_size`, `max_token`, etc. (see `load_alpaca_data` / `load_contrastive_data` in `main.py` and `datas.py`).
3. Strict reproduction may require the same **seeds, data subset, and step count** as the paper. The code only sets `seed=42` on some shuffles; for full reproducibility, also fix `torch` / `numpy` / `random` seeds.
...
```bibtex
% TODO: replace placeholders with the official title, authors, venue, and year
@article{sap2025,
  title   = {Your SAP Paper Title},
  author  = {...},
  journal = {or booktitle = {...} for conference},
  year    = {2025}
}
```
...
- **This repository:** Unless otherwise stated, authors may choose MIT, Apache-2.0, etc. Add a root `LICENSE` file to make redistribution terms explicit.
- **Third parties:** Llama 2, HuggingFace data, and dependencies have their own licenses; users must comply with them.
